**ID:** BUG-10\
**Severity:** Major\
**Priority:** Medium
## Core Information
*   **Bug Type:** Functional
*   **Build Version:** N/A
*   **Environment:** Laptop / Windows 11 home 24H2 / Google Chrome 140.0.7339.128 / Screen resolution 1920x1080
## Description
\[Products\] Rating stars missing when submitting review.
## Preconditions
N/A
## Steps To Reproduce (STR)
1.  Navigatate to product page (https://automationexercise.com/products)
2.  Choose any product, click View Product
3.  Navigate the UI to locate star rating option
## Expected Result (ER)
User can select a star rating when submitting a review.
## Actual Result (AR)
Rating stars are missing / cannot be selected.
## Attachments
-  N/A
## Responsibility
-   **Reporter:** Rafayel Khachatryan
-   **Assigned To:** Unassigned
-   **Date Created:** 2025-09-17
